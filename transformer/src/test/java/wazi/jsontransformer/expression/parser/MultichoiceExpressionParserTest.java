...
public class MultichoiceExpressionParserTest {
	...
	public void testReadExpression() throws Exception {
		...
		Expression ex1 = parser.readExpression(new JTEX("1234e13adf"));
		assertEquals(0, ex1.getStart());
		assertEquals(6, ex1.getEnd());
		assertEquals(1234e13, ex1.eval(null));

		Expression ex2 = parser.readExpression(new JTEX("\"Hello \\\"World\\\"\\u3287\""));
		...
		Expression ex5 = parser.readExpression(new JTEX("true"));
	...
	public void test2() {
		...
		Expression expression = parser.readExpression(jtex);
